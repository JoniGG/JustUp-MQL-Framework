//--- This library will allow you to login to a new trading account from the code.
//--- Credits to @sergeylukin in Github
...
//--- Login to a new account from this function
void LoginToAccount(string accountNumber, string accountPassword, string serverHostname)
{
    if(loginDialogIsOpen()) closeLoginDialog();

    if(auth(accountNumber, accountPassword, serverHostname))
    {
        Print("Account authentication done successfully!");
    }
    else
    {
        Print("An error ocurred while authenticating to the account.");
    }
}
